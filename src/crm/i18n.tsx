...
export const CRM_T: Record<Lang, any> = {
  ...
  ru: {
    ...
    sub: { recommended: 'Рекомендуем', upTo: 'До', staffWord: 'сотрудников', perMonth: 'в месяц', tooSmall: 'Мало для вашей команды', choose: 'Выбрать', choosePlan: 'Выбрать тариф', trialOverTitle: 'Бесплатный месяц закончился', expiredTitle: 'Подписка закончилась', trialOverSub: 'Выберите тариф, чтобы продолжить работу.', expiredSub: 'Продлите тариф, чтобы вернуть доступ к CRM.', lapsedDays: (n: number) => `Закончилась ${n} дн. назад.`, trialEndsIn: (n: number) => `Бесплатный месяц заканчивается через ${n} дн.`, planEndsIn: (n: number) => `Подписка заканчивается через ${n} дн.`, pasteInChat: 'Сообщение уже в чате — просто отправьте его. Если поле пустое, вставьте из буфера.', tellManager: 'Напишите в чате название бизнеса и выбранный тариф.', bookingStillWorks: 'Страница записи для клиентов и Telegram-боты продолжают работать — остановлен только CRM.' },
  ...
  en: {
    ...
    sub: { recommended: 'Recommended', upTo: 'Up to', staffWord: 'staff', perMonth: 'per month', tooSmall: 'Too small for your team', choose: 'Choose', choosePlan: 'Choose a plan', trialOverTitle: 'Your free month is over', expiredTitle: 'Your subscription has expired', trialOverSub: 'Choose a plan to carry on working.', expiredSub: 'Renew to get your CRM back.', lapsedDays: (n: number) => `Expired ${n} days ago.`, trialEndsIn: (n: number) => `Your free month ends in ${n} days.`, planEndsIn: (n: number) => `Your subscription ends in ${n} days.`, pasteInChat: 'The message is already in the chat — just send it. If the box is empty, paste it.', tellManager: 'Tell them your business name and the plan you picked.', bookingStillWorks: 'Your customers’ booking page and the Telegram bots are still running — only the CRM is paused.' },
